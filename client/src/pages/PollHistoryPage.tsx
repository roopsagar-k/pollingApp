...
type BackendPoll = {
  ...
  questions?: BackendQuestion[]; // Add this
};
...
export const PollHistoryPage: React.FC = () => {
  ...
  const questions = useMemo(() => {
    ...
    polls.forEach((poll) => {
      // Add all questions from the questions array
      if (poll.questions && poll.questions.length > 0) {
        allQuestions.push(...poll.questions);
      }
      // Add the current question if it exists
      if (poll.currentQuestion) {
        allQuestions.push(poll.currentQuestion);
      }
    });
